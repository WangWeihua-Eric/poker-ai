...
export class WpkProService {
    ...
    switch (round) {
      ...
      default:
        ...
        const leftCardWithDeleteColor =
          this.deleteColor(leftCard) === '01'
            ? 14
            : parseInt(this.deleteColor(leftCard), 10);
        const rightCardWithDeleteColor =
          this.deleteColor(rightCard) === '01'
            ? 14
            : parseInt(this.deleteColor(rightCard), 10);
        if (
          (leftCardWithDeleteColor > 12 && rightCardWithDeleteColor > 12) ||
          (leftCardWithDeleteColor === rightCardWithDeleteColor &&
            leftCardWithDeleteColor === 12)
        ) {
          maxCards = true;
        }
        break;
      ...
      case 'FLOP':
        ...
        switch (cardType) {
          case CardTypeEnum.HIGH_CARD:
            if (
              !this.checkFlush(dealPublicCards) &&
              !this.checkStraight(dealPublicCards) &&
              !overPot
            ) {
              if (
                this.checkMeBuyFlush(allCards) ||
                (this.checkMeBuyStraight(allCards) &&
                  !this.checkBuyFlush(dealPublicCards) &&
                  !this.checkStraight(dealPublicCards))
              ) {
                cardFaceGood = true;
              }
          ...
          case CardTypeEnum.ONE_PAIRS:
            if (
              !this.checkCommonPairs(dealPublicCards) &&
              !this.checkFlush(dealPublicCards) &&
              !this.checkStraight(dealPublicCards) &&
              !this.checkHats(dealPublicCards, handCards) &&
              !overPot
            ) {
              // 检查是否是公对、花面、顺面
              cardFaceGood = true;
      ...
      default:
        ...
        if (overPot) {
          if (leftCardWithDeleteColor > 12 && rightCardWithDeleteColor > 12) {
            ...
            leftCardWithDeleteColor === rightCardWithDeleteColor &&
            rightCardWithDeleteColor === 12
          ) {
            cardFaceGood = true;
          }
        } else if (
          leftCardWithDeleteColor > 9 &&
          rightCardWithDeleteColor > 9
        ) {
          cardFaceGood = true;
          ...
          if (
            ...
            leftCardWithDeleteColor > 8 &&
            rightCardWithDeleteColor > 8
          ) {
            cardFaceGood = true;
          } else if (
            Math.abs(leftCardWithDeleteColor - rightCardWithDeleteColor) ===
              1 &&
            leftCardWithDeleteColor > 4 &&
            rightCardWithDeleteColor > 4
          ) {
            cardFaceGood = true;
          } else if (
            Math.abs(leftCardWithDeleteColor - rightCardWithDeleteColor) ===
              2 &&
            leftCardWithDeleteColor + rightCardWithDeleteColor === 18
          ) {
            cardFaceGood = true;
          }
        } else if (
  ...
  checkMeBuyStraight(cards) {
    ...
    for (let i = 0; i < cardsWithDeleteColor.length - 3; i++) {
      if (
        cardsWithDeleteColor[i] + 1 === cardsWithDeleteColor[i + 1] &&
        cardsWithDeleteColor[i] + 2 === cardsWithDeleteColor[i + 2] &&
        cardsWithDeleteColor[i] + 3 === cardsWithDeleteColor[i + 3]
      ) {
        meBuyStraight = true;
      }
    }
  ...
  checkHats(dealPublicCards, myCards) {
    ...
    return ![
      parseInt(this.deleteColor(myCards[0]), 10),
      parseInt(this.deleteColor(myCards[1]), 10),
    ].includes(maxPublicCard);
  }
  ...
  checkBuyStraight(dealPublicCards) {
    ...
    for (let i = 0; i < publicCardsWithDeleteColor.length - 1; i++) {
      for (let j = 1; j < publicCardsWithDeleteColor.length; j++) {
        const diff = Math.abs(
          publicCardsWithDeleteColor[i] - publicCardsWithDeleteColor[j]
        );
        if (diff < 3 && diff > 0) {
          buyStraight = true;
        }
  ...
  checkStraight(dealPublicCards) {
    ...
    for (let i = 0; i < publicCardsWithDeleteColor.length - 2; i++) {
      if (
        publicCardsWithDeleteColor[i] + 1 ===
          publicCardsWithDeleteColor[i + 1] &&
        publicCardsWithDeleteColor[i] + 2 === publicCardsWithDeleteColor[i + 2]
      ) {
        straight = true;
      } else if (
        publicCardsWithDeleteColor[i] + 2 ===
          publicCardsWithDeleteColor[i + 1] &&
        publicCardsWithDeleteColor[i] + 3 === publicCardsWithDeleteColor[i + 2]
      ) {
        straight = true;
      } else if (
        publicCardsWithDeleteColor[i] + 3 ===
          publicCardsWithDeleteColor[i + 1] &&
        publicCardsWithDeleteColor[i] + 4 === publicCardsWithDeleteColor[i + 2]
      ) {
        straight = true;
      } else if (
        publicCardsWithDeleteColor[i] + 1 ===
